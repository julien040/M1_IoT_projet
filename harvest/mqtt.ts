...
import type { TravelTimeResponse } from "./googleMaps.ts";
...
let voiture: TravelTimeResponse | null = null;
...
const loop = async () => {
    ...
    // Rafraichissement complet toutes les 10 itérations (20 minutes)
    if (i % 40 === 0) {
        console.log("Rafraichissement complet des données voiture...");

        voiture = await getTravelTime(
            origin,
            destination,
            process.env.MAPS_API_KEY!
        );
    ...
    tempsBus.then((bus) => {
        ...
        if (bus.tempsTotalMinute === Infinity) {
            ...
            console.log(
                `Prochains départs bus (minutes): ${bus.prochainsDeparts.join(
                    ", "
                )}`
            );
        }
