...
const origin = "7 Avenue Henri Dunant, 64000 Pau, France";
...
function main() {
    ...
    setInterval(() => {
        ...
        const tempsVoiture = getTravelTime(origin, destination, process.env.MAPS_API_KEY!);
        const bus: etapeBus = {
        ...
        Promise.all([tempsVoiture, tempsBus]).then(([voiture, bus]) => {
            ...
            const messageVoiture = `${voiture ? (voiture.dureeSeconde / 60).toFixed(0) : "N/A"} min ${voiture ? (voiture.dureeSeconde % 60) : "N/A"} sec`;
            const messageBus = bus.tempsTotalMinute === Infinity ? "N/A" : `${bus.tempsTotalMinute} min`;
            const messageDeparts = bus.prochainsDeparts.length > 0 ? bus.prochainsDeparts.splice(0, 5).join("min, ") : "N/A";
    ...
    }, 5000); // Raffraichit toutes les 30 secondes
}